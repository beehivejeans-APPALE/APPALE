...
- `status` の `draft → in_review` の遷移は実装済み（[`docs/05_PUBLISH.md`](05_PUBLISH.md)）。`in_review → published` は本人の `update` 権限では変更できないようにアプリ側で制御しており、現時点ではSupabase側の手動運用としている。将来的に運営用の承認画面を用意する場合は別途設計する。
